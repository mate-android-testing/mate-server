...
public class Device {
    ...
    /**
     * The files that can be generated by the tracer.
     */
    private enum TracerFile {

        /**
         * The info.txt is generated once the tracer finished dumping its traces. It contains the number of collected
         * traces. Note that this file is only generated after a store request via a broadcast, not when the tracer
         * dumps the traces because it reached the cache limit.
         */
        INFO,

        /**
         * The traces.txt contains the dumped traces. Generated via a broadcast request or when the tracer reached the
         * cache limit.
         */
        TRACES,

        /**
         * The running.txt is generated whenever the tracer begins to dump traces. Removed upon completion.
         */
        RUNNING;
         ...
         *
         * @return Returns the corresponding file name.
         */
        @Override
    ...
    /**
     * Checks which one of the tracer's files exist.
     *
     * @return Returns a set containing the existing tracer files.
     */
    private Set<TracerFile> existingTracerFiles() {

        final var query =
                ProcessRunner.runProcess(androidEnvironment.getAdbExecutable(), "-s", deviceID, "shell", "ls", EXTERNAL_STORAGE);

        if (query.isOk()) {
            final var files = query.getOk();
            return Arrays.stream(TracerFile.values())
                    .filter(file -> files.stream()
                            .anyMatch(str -> str.trim().equals(file.toString())))
                    .collect(toSet());
        } else {
            throw new IllegalStateException("Cannot get status of traces file: '" + query.getErr() + "'");
        }
    }

    /**
     * Returns whether the tracer is currently dumping its traces.
     *
     * @return Returns {@code true} if the tracer is currently dumping its traces, otherwise {@code false}.
     */
    private boolean isTracerRunning() {
        return existingTracerFiles().contains(TracerFile.RUNNING);
    }

    /**
     * Waits for the tracer to finish dumping the traces.
     *
     * @return Returns {@code true} if the tracer finished dumping its traces, otherwise {@code false} is returned.
     */
    private boolean waitForTracerToFinish() {
        ...
        if (isTracerRunning()) {
            ...
            final long endTime = startTime + maxWaitTimeInSeconds * 1000;
            while ((currentTime = System.currentTimeMillis()) < endTime) {
                Util.sleep(1);
                if (!isTracerRunning()) {
                    Log.println(String.format("Tracer done, waited for %dms", currentTime - startTime));
                    break;
                }
            }

            if (isTracerRunning()) {
                Log.println("Waiting for Tracer exceeded max wait time.");
                return false;
            } else {
                return true;
            }
    ...
    private void getTracesFromTracer() {
        ...
        final var existingFiles = existingTracerFiles();

        if (existingFiles.contains(TracerFile.INFO) && !existingFiles.contains(TracerFile.TRACES)) {
            throw new IllegalStateException("info.txt exists, but not traces.txt, this should not happen.");
        }

        if (!existingFiles.contains(TracerFile.INFO)) {
